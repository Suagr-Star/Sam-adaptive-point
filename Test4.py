...
def init_per_image_csv():
    """初始化单张图片结果CSV，创建表头（四项检测+五种提示mIoU）"""
    if not os.path.exists(Config.PER_IMAGE_CSV_PATH):
        with open(Config.PER_IMAGE_CSV_PATH, "w", encoding="utf-8", newline="") as f:
            fieldnames = [
                ...
                "Box-only_mIoU",
                "Box-Point(Original)_mIoU",
                "Box-Point(Adaptive)_mIoU"
            ]
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
    print(f"单张图片五项提示结果将保存至：{Config.PER_IMAGE_CSV_PATH}（追加模式，不覆盖）")
...
def append_per_image_result(img_data):
    ...
    with open(Config.PER_IMAGE_CSV_PATH, "a", encoding="utf-8", newline="") as f:
        fieldnames = [
            ...
            "Point-only_mIoU", "Point(Adaptive)_mIoU",
            "Box-only_mIoU", "Box-Point(Original)_mIoU", "Box-Point(Adaptive)_mIoU"
        ]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
...
def build_prompts(img_info, ann, coco, predictor, size_thresh, shape_thresh):
    """
    自适应选点版本：返回各类提示所需数据
    返回：原始点/自适应点/框/未优化Box-Point所需数据
    """
    img_w, img_h = img_info["width"], img_info["height"]
    bbox = ann["bbox"]
    ...
    # 3. 原始质心点（Box-Point(Original)用，仅正点）
    box_point_original = np.array([[pos_x, pos_y]], dtype=np.float32)
    box_point_original_labels = np.array([1], dtype=np.int32)
    
    # 4. 自适应点（Point(Adaptive)和Box-Point(Adaptive)用，仅正点）
    size_feat = get_size_feature(ann, img_w, img_h)
    shape_feat = get_shape_feature(mask)
    occlusion_feat = get_occlusion_feature(ann)
    instance_type = classify_instance(size_feat, shape_feat, occlusion_feat, size_thresh, shape_thresh)
    ...
    # 封装返回
    return (original_points, original_point_labels), \
           (adaptive_points, adaptive_labels), \
           (box_xyxy,), \
           (box_point_original, box_point_original_labels), \
           (adaptive_points, adaptive_labels, box_xyxy)
...
def run_exp1():
    ...
    log_content.append(f"实验1：5种SAM提示方式性能对比（含复杂图判断）")
    ...
    for idx, img_id in enumerate(selected_img_ids):
        ...
        for ann in anns:
            # 构建所有提示数据
            point_only_data, point_adaptive_data, box_only_data, box_point_original_data, box_point_adaptive_data = build_prompts(
            ...
            img_instance_iou["Point-only"].append(iou)
            result_dict["Point-only"]["instance_iou_list"].append(iou)
            
            # （2）Point(Adaptive) 推理（仅自适应点，无box）
            ...
            pred_mask_scaled = scale_mask(pred_mask, img_h, img_w)
            iou = calculate_iou(pred_mask_scaled, gt_mask)
            img_instance_iou["Point(Adaptive)"].append(iou)
            result_dict["Point(Adaptive)"]["instance_iou_list"].append(iou)
            
            # （3）Box-only 推理（仅框，无点）
            box_xyxy, = box_only_data
            with torch.no_grad():
                if config.DEVICE == "cuda":
                    with torch.cuda.amp.autocast():
                        masks, scores, _ = predictor.predict(
            ...
            pred_mask_scaled = scale_mask(pred_mask, img_h, img_w)
            iou = calculate_iou(pred_mask_scaled, gt_mask)
            img_instance_iou["Box-only"].append(iou)
            result_dict["Box-only"]["instance_iou_list"].append(iou)
            
            # （4）Box-Point(Original) 推理（框+原始质心点，未优化）
            box_point_pts, box_point_labels = box_point_original_data
            with torch.no_grad():
                if config.DEVICE == "cuda":
                    with torch.cuda.amp.autocast():
                        masks, scores, _ = predictor.predict(
                            point_coords=box_point_pts,
            ...
            pred_mask_scaled = scale_mask(pred_mask, img_h, img_w)
            iou = calculate_iou(pred_mask_scaled, gt_mask)
            img_instance_iou["Box-Point(Original)"].append(iou)
            result_dict["Box-Point(Original)"]["instance_iou_list"].append(iou)
            
            # （5）Box-Point(Adaptive) 推理（框+自适应点，优化后）
            adaptive_pts, adaptive_labs, mixed_box = box_point_adaptive_data
            with torch.no_grad():
                if config.DEVICE == "cuda":
                    with torch.cuda.amp.autocast():
                        masks, scores, _ = predictor.predict(
                            point_coords=adaptive_pts,
                            point_labels=adaptive_labs,
                            box=mixed_box,
                            multimask_output=True
                        )
                else:
            ...
            pred_mask_scaled = scale_mask(pred_mask, img_h, img_w)
            iou = calculate_iou(pred_mask_scaled, gt_mask)
            img_instance_iou["Box-Point(Adaptive)"].append(iou)
            result_dict["Box-Point(Adaptive)"]["instance_iou_list"].append(iou)
        ...
        img_detail_data = {
            ...
            "Point(Adaptive)_mIoU": current_img_miou["Point(Adaptive)"],
            "Box-only_mIoU": current_img_miou["Box-only"],
            "Box-Point(Original)_mIoU": current_img_miou["Box-Point(Original)"],
            "Box-Point(Adaptive)_mIoU": current_img_miou["Box-Point(Adaptive)"]
        }
        ...
        if (idx + 1) % 10 == 0 or (idx + 1) == total_img:
            ...
            print(f"  Point(Adaptive) mIoU：{current_img_miou['Point(Adaptive)']:.4f}")
            print(f"  Box-only mIoU：{current_img_miou['Box-only']:.4f}")
            print(f"  Box-Point(Original) mIoU：{current_img_miou['Box-Point(Original)']:.4f}")
            print(f"  Box-Point(Adaptive) mIoU：{current_img_miou['Box-Point(Adaptive)']:.4f}")
            print("-"*50)
    ...
    print(f"单张图片五项提示结果已保存至：{config.PER_IMAGE_CSV_PATH}")
    
    # 打印最终结果（5种提示方式对比）
    print("\n" + "="*80)
    print("实验1 最终结果对比（5种提示方式+复杂图判断）")
    print("="*80)
    ...
    # 关键对比结论
    adaptive_box_point_miou = final_results[4]["实例级mIoU"]
    original_box_point_miou = final_results[3]["实例级mIoU"]
    box_only_miou = final_results[2]["实例级mIoU"]
    point_adaptive_miou = final_results[1]["实例级mIoU"]
    point_only_miou = final_results[0]["实例级mIoU"]
    
    print(f"\n关键对比结论：")
    print(f"1. Point(Adaptive) 对比 Point-only：提升 {point_adaptive_miou - point_only_miou:.4f}")
    print(f"2. Box-Point(Adaptive) 对比 Box-Point(Original)：提升 {adaptive_box_point_miou - original_box_point_miou:.4f}")
    print(f"3. Box-Point(Adaptive) 对比 Box-only：{'提升 ' + str(adaptive_box_point_miou - box_only_miou):.4f}")
    # 打印复杂图统计
